...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt_long(argc, argv, "hm:o:", long_options, &option_index)) != -1) {
        switch (c) {
            case 'h':
                print_help();
                return 0;
            ...
            case 'm':
                ...
                // Remove me once model 3 is implemented!
                if (m == 3) {
    ...
    if (pi.init()){
        std::cerr << "Error importing TranscriptHits Python module.  Please check your PYTHONPATH\n";
        return 1;
    }
    ...
    t1 = clock();
    SampleAllelicExpression sae(aim);
    t2 = clock();
    ...
    do {
        sae.update(model);
    } while (!sae.converged() && ++num_iterations < max_iterations);
    t2 = clock();
